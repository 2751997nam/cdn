...
router.get('/images/:resolution(\\d+x\\d+)/:quality(\\d+)?/:fitIn(fit-in)?/:url(*(.png|.jpeg|.jpg|.webp))', ResizeImageController.resize.bind(ResizeImageController));
router.get('/images/:resolution(\\d+x\\d+)/:quality(\\d+)?/:fitIn(fit-in)?/:url(*)', ResizeImageController.redirectUnsuportImage.bind(ResizeImageController));
